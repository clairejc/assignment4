...
beforeEach(async () => {
  ...
  await app.createUser(getEmptySession(), "alice", "alice123", "hi", 11);
  await app.createUser(getEmptySession(), "bob", "bob123", "bye", 111);
});
...
describe("Create a user and log in", () => {
  it("should create a user and log in", async () => {
    ...
    const created = await app.createUser(session, "barish", "1234", "hello", 1);
    assert(created.user);
  ...
  it("duplicate username should fail", async () => {
    ...
    const created = await app.createUser(session, "barish", "1234", "hello", 3);
    assert(created.user);
    await assert.rejects(app.createUser(session, "barish", "1234", "bye", 11));
  });
